...
export class PokemonsService {
  ...
  determineTypeModifier(attacker: Pokemon, defender: Pokemon): number {
    if(attacker['Type 1'] === 'electric' || attacker['Type 2'] === 'electric') {
      if (defender['Type 1'] === 'water' || defender['Type 2'] === 'water') return 2;
      if (defender['Type 1'] === 'rock' || defender['Type 2'] === 'rock') return 0.5;
    }
    return 1;
  }
  ...
  async calculateDamage(attackerId: number, defenderId: number): Promise<number> {
    ...
    const typeModifier = this.determineTypeModifier(attacker, defender);
